...
_geo = GeoEnricher()
raw = _geo.enrich_dataframe(
    raw, ip_col="src_ip", country_col="geo_country_resolved", asn_col="asn_resolved"
)
print(
    f"  Resolved {(~raw['asn_resolved'].isin(['UNRESOLVED'])).mean():.1%} of rows "
...
raw["input_output_ratio"] = raw.apply(
    lambda row: (
        row["total_input_btc"] / row["total_output_btc"]
        if row["total_output_btc"] != 0
        else 0
    ),
    axis=1,
)
...
raw["fee_ratio"] = raw.apply(
    lambda row: (
        row["fee"] / row["total_input_btc"] if row["total_input_btc"] > 1e-12 else 0.0
    ),
    axis=1,
)
...
for addr, idxs in raw.groupby("first_input_addr").groups.items():
    ...
    for i in idxs:
        ...
        raw.at[i, "sender_tx_count_1h"] = sum(
            1 for pt in recent_times if t - pt <= ONE_HOUR
        )
...
# ---- Merge labels ----
df = featured.merge(
    labels[["txid", "is_anomaly", "anomaly_type", "wallet_ids"]], on="txid"
)
assert len(df) == len(featured), (
    "Row count changed on merge - investigate before continuing"
)
...
for atype in anomaly_df["anomaly_type"].unique():
    wallets = sorted(
        anomaly_df[anomaly_df["anomaly_type"] == atype]["canonical_wallet_id"]
        .unique()
        .tolist()
    )
...
assert len(overlap) == 0, (
    f"LEAKAGE: {len(overlap)} canonical wallets in both train and test"
)
...
print(
    f"Train shape: {train_df.shape}, anomaly rate: {train_df['is_anomaly'].mean():.4f}"
)
print(f"Test shape:  {test_df.shape}, anomaly rate: {test_df['is_anomaly'].mean():.4f}")
print(
    f"Train canonical wallets: {len(train_groups)}, Test canonical wallets: {len(test_groups)}, Canonical wallet overlap: {len(overlap)}"
)
