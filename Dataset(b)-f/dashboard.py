...
# NOTE: the generator for this specific file is currently unresolved.
# No script in this pipeline (entity_graph_clustered_.py included) writes
# entity_graph_ip_tx_wallet.graphml -- entity_graph_clustered_.py's output
# graph has a different schema entirely (no transaction nodes; transactions
# are edge attributes between wallet/IP nodes, using is_ip/is_disposable/
# max_priority_tier instead of node_type/label/priority_tier). Until the
# real generator is confirmed or this section is rewritten against that
# other schema, this stays pointed at a file that won't exist -- the code
# below handles that gracefully (warns, doesn't crash) rather than masking
# it.
...
# Additional graph views -- these two ARE correctly wired to real
# generator scripts (entity_graph_clustered_.py and entity_graph_wallet_.py
# respectively), unlike GRAPHML_FILE above.
...
def load_wallet_graph():
    """Load the wallet -> wallet graph generated by entity_graph_wallet_.py."""
...
def render_wallet_graph(G_wallet, selected_wallet=None):
    ...
    for node, data in graph.nodes(data=True):
        ...
        if node_id == str(selected_wallet):
            # Same rule as render_investigation_graph: highlight uses the
            # accent blue, not a red, so it can't be mistaken for a
            # high-priority tier color.
            color = "#4A9FD8"
            size = 34
        else:
            color = TIER_COLORS.get(priority, TIER_COLORS["none"])
...
def show_graph_explorer():
    ...
    if graph_choice == "IP → Transaction → Wallet":
        if GRAPH_HTML_FILE.exists():
            ...
            st.caption("Run the IP -> Transaction -> Wallet graph builder to regenerate it.")
        else:
            st.warning(
                "IP → Transaction → Wallet graph not found. The script that "
                "generates entity_graph_ip_tx_wallet.graphml hasn't been "
                "confirmed for this pipeline yet -- see the note near "
                "GRAPHML_FILE at the top of this file."
            )
        ...
        if RISK_GRAPH_HTML_FILE.exists():
            ...
            st.warning(
                "Risk Cluster graph not found. "
                "Run entity_graph_clustered_.py first."
            )
        ...
        if wallet_graph is None:
            st.warning(
                "Wallet → Wallet graph not found. "
                "Run entity_graph_wallet_.py first."
            )
            return
...
if len(filtered) == 0:
    ...
    if len(selected) == 0:
        ...
        if investigation_graph is not None:
            ...
            st.warning(
                "Entity graph not found. The generator for "
                "entity_graph_ip_tx_wallet.graphml hasn't been confirmed for "
                "this pipeline -- see the note at GRAPHML_FILE, top of file."
            )
...
if G is None:
    st.warning(
        "Entity graph could not be loaded. The generator for "
        "entity_graph_ip_tx_wallet.graphml hasn't been confirmed for this "
        "pipeline -- see the note at GRAPHML_FILE, top of file."
    )
else:
