...
# GraphML generated by entity_graph.py
...
def build_investigation_graph(txid, wallet_id):
    ...
    if wallet_node is not None:
        ...
        if G.is_directed():
            ...
            nodes_to_keep.update(
                G.predecessors(wallet_node)
            )

    investigation_G = G.subgraph(
        nodes_to_keep
    ).copy()
    ...
    if wallet_node is not None:
        ...
        if not (
            ...
            investigation_G.add_edge(
                tx_node,
                wallet_node,
                label="belongs_to_wallet"
            )

    return investigation_G

# Bug (tech debt): these four queries used to be reimplemented here, a
# second, drifting copy of the exact same logic already written and
# verified in investigation_queries.py. That module's versions raise
# ValueError when an entity isn't found; these thin wrappers just catch
# that and return None instead, so every "if x is None: st.error(...)"
# call site below didn't need to change.
from investigation_queries import (
    n_hop_query as _iq_n_hop_query,
    shortest_path_query as _iq_shortest_path_query,
    fund_flow_query as _iq_fund_flow_query,
    get_path_subgraph,
)
...
def n_hop_query(G, entity, hops=2):
    try:
        return _iq_n_hop_query(G, str(entity), hops=hops)
    except ValueError:
        return None
...
def shortest_path_query(G, entity_a, entity_b):
    try:
        return _iq_shortest_path_query(G, str(entity_a), str(entity_b), directed=False)
    except ValueError:
        return None
...
def fund_flow_query(G, entity, hops=2, direction="both"):
    try:
        return _iq_fund_flow_query(G, str(entity), hops=hops, direction=direction)
    except ValueError:
        return None
...
def render_investigation_graph(G, selected_txid):
    ...
    # Computed ONCE here, not per-node inside the loop below -- previously
    # this scanned the entire graph again for every single node just to
    # check "is this the highlighted one", which is O(n^2) and only got
    # away with it because focused subgraphs stayed ~100 nodes. Breaks down
    # as soon as --hops (or the slider) goes up.
    highlighted_tx_node = str(find_transaction_node(G, selected_txid))
    ...
    for node, data in G.nodes(data=True):
        ...
        if node_id == highlighted_tx_node:
            ...
            # NOTE: wallet nodes carry their tier under "max_priority_tier"
            # (a wallet can have several alerted transactions; this is the
            # worst one) -- entity_graph.py only ever sets a plain
            # "priority_tier" on TRANSACTION nodes. Reading "priority_tier"
            # here always misses on wallet nodes, so every wallet silently
            # fell through to blue regardless of alert status.
            priority = str(
                data.get(
                    "max_priority_tier",
                    ""
            ...
            # Only "high" / "medium-high" / "worth reviewing" are ever
            # produced by build_alerts.py -- "critical" doesn't exist in
            # this pipeline and was dead code.
            if priority == "high":
        ...
        if "priority_tier" in data:
            ...
            tooltip += (
                f"<br>Priority: "
                f"{data['priority_tier']}"
            )

        if "max_priority_tier" in data:

            tooltip += (
                f"<br>Worst alert tier: "
                f"{data['max_priority_tier']}"
            )
...
high_alerts = len(
    alerts[
        alerts["priority_tier"] == "high"
    ]
)

medium_alerts = len(
    alerts[
        alerts["priority_tier"] == "medium-high"
    ]
)

both_alerts = len(
    alerts[
        alerts["detector"] == "both"
    ]
)
...
table["xgb_proba"] = table[
    "xgb_proba"
].round(4)

table["if_score"] = table[
    "if_score"
].round(4)
...
if len(filtered) == 0:
    ...
    selected_txid = st.selectbox(
        "Select a transaction for this wallet",
        wallet_filtered["txid"].tolist()
    )
    ...
    if len(selected) == 0:
        ...
        if investigation_graph is not None:
            ...
            graph_html = GRAPH_HTML_FILE.read_text(
                encoding="utf-8"
            )
...
if G is None:
    st.warning("Entity graph could not be loaded. Run entity_graph.py first.")
else:
