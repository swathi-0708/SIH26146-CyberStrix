...
FEATURE_COLS = [
    ...
    "fee",
    "fee_ratio",
    "input_addr_is_recent_output",
    "minutes_since_addr_last_output",
    "sender_tx_count_1h",
    "sender_tx_count_24h",
...
FEATURE_PHRASING = {
    ...
    "n_unique_input_addresses": (
        "many distinct input addresses",
        "few distinct input addresses",
    ),
    "n_unique_output_addresses": (
        "many distinct output addresses",
        "few distinct output addresses",
    ),
    "total_input_btc": ("large total BTC moved", "small total BTC moved"),
    "input_output_ratio": (
        "input/output amount mismatch",
        "near-perfect input/output balance",
    ),
    ...
    "input_addr_is_recent_output": (
        "input addr reused right after receiving",
        "input addr not recently active",
    ),
    "minutes_since_addr_last_output": (
        "very fast address reuse",
        "long dormancy before reuse",
    ),
    ...
    "sender_time_since_last_tx_min": (
        "rapid repeat transacting",
        "long gap since last tx",
    ),
    "sender_distinct_asn_last10": (
        "hopping across many ASNs recently",
        "consistent ASN recently",
    ),
    "sender_distinct_ip_last10": (
        "hopping across many IPs recently",
        "consistent IP recently",
    ),
    "sender_amount_zscore": (
        "amount far from sender's usual pattern",
        "amount close to sender's usual pattern",
    ),
}
...
def main():
    ...
    for i, txid in enumerate(alert_rows.index):
        ...
        reasons[txid] = {
            ...
            "top_feature_1": FEATURE_COLS[top_idx[0]],
            "shap_1": round(float(row_shap[top_idx[0]]), 4),
            "top_feature_2": FEATURE_COLS[top_idx[1]],
            "shap_2": round(float(row_shap[top_idx[1]]), 4),
            "top_feature_3": FEATURE_COLS[top_idx[2]],
            "shap_3": round(float(row_shap[top_idx[2]]), 4),
        }

    reasons_df = (
        pd.DataFrame.from_dict(reasons, orient="index")
        .reset_index()
    ...
    print(
        f"Explained {(~no_xgb).sum()} of {len(explained)} alerts via SHAP "
