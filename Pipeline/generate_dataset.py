...
def rand_fee_rate():
    ...
    Using one distribution across normal and anomaly generators prevents
...
class WalletProfile:
    ...
    def __init__(self, wid):
        ...
        self.avg_gap_minutes = random.choice(
            [30, 60, 180, 720, 1440, 4320]
        )  # 30min .. 3days
...
def make_normal_transaction(wallets, t, wid_from=None):
    ...
    amount = max(
        0.00001, np.random.normal(sender.typical_amount, sender.typical_amount * 0.25)
    )
...
def inject_peeling_chain(wallets, start_time, chain_len=8):
    ...
    for hop in range(chain_len):
        peel = round(
            current_amount * np.random.uniform(0.02, 0.08), 8
        )  # small peeled-off amount
...
def inject_fan_out_mixer(wallets, start_time, n_in=25, n_out=25):
    ...
    rows.append(
        {
            "timestamp": t.isoformat() + "Z",
            "src_ip": mixer.home_ip,
            "dst_ip": rand_ip(mixer.asn_entry),
            "src_port": random.randint(1024, 65535),
            "dst_port": 8333,
            "txid": txid_in,
            "input_addresses": input_addrs,
            "output_addresses": [mixer.address],
            "input_amounts": [round(total_in / n_in, 8)] * n_in,
            "output_amounts": [
                round(total_in - round(total_in * rand_fee_rate() + 0.00001, 8), 8)
            ],
            "fee": round(total_in * rand_fee_rate() + 0.00001, 8),
            "script_type": mixer.script_type,
            "geo_country": mixer.asn_entry["country"],
            "asn": mixer.asn_entry["asn"],
        }
    )
    labels.append(
        {
    ...
    for i in range(n_out):
        ...
        rows.append(
            {
                "timestamp": t.isoformat() + "Z",
                "src_ip": rand_ip(out_asn),
                "dst_ip": fake.ipv4_public(),
                "src_port": random.randint(1024, 65535),
                "dst_port": 8333,
                "txid": txid_out,
                "input_addresses": [mixer.address],
                "output_addresses": [new_wallet_address()],
                "input_amounts": [per_out],
                "output_amounts": [round(per_out - out_fee, 8)],
                "fee": out_fee,
                "script_type": random.choice(SCRIPT_TYPES),
                "geo_country": out_asn["country"],
                "asn": out_asn["asn"],
            }
        )
        labels.append(
        ...
        labels.append(
            {
                "txid": row["txid"],
                "is_anomaly": 1,
                "anomaly_type": "wallet_reuse_burst",
                "wallet_ids": f"burst_{w.id}",
            }
        )
        t += timedelta(
            seconds=random.randint(20, 90)
        )  # far faster than its normal cadence
    return rows, labels
...
def inject_structuring(wallets, start_time, threshold=10.0, n_tx=12):
    ...
    t = start_time
    for i in range(n_tx):
        # Threshold drawn from the normal upper tail (lognormal(-1.8, 0.7), median ~0.165 BTC)
        # so normal transactions can also legitimately fall near this threshold.
        # The anomaly signal is the PATTERN of repeated near-threshold txs, not the amount.
        threshold = max(round(np.random.lognormal(mean=-1.8, sigma=0.7), 6), 0.02)
        amount = round(
            threshold * (1 - np.random.uniform(0.005, 0.05)), 8
        )  # 0.5–5% below threshold
        ...
        rows.append(
            {
                "timestamp": t.isoformat() + "Z",
                "src_ip": w.home_ip,
                "dst_ip": rand_ip(w.asn_entry),
                "src_port": random.randint(1024, 65535),
                "dst_port": 8333,
                "txid": txid,
                "input_addresses": [w.address],
                "output_addresses": [receiver_addr],
                "input_amounts": [amount],
                "output_amounts": [round(amount - struct_fee, 8)],
                "fee": struct_fee,
                "script_type": w.script_type,
                "geo_country": w.asn_entry["country"],
                "asn": w.asn_entry["asn"],
            }
        )
        labels.append(
...
def inject_ip_hopping(wallets, start_time, n_tx=15):
    ...
    t = start_time
    for i in range(n_tx):
        hop_asn = random.choice(
            ASN_POOL
        )  # different ASN each time -> unstable network origin
        receiver_addr = new_wallet_address()
        ...
        rows.append(
            {
                ...
                "output_addresses": [receiver_addr],
                "input_amounts": [amount],
                "output_amounts": [round(amount - hop_fee, 8)],
                "fee": hop_fee,
                "script_type": w.script_type,
                "geo_country": hop_asn["country"],
                "asn": hop_asn["asn"],
            }
        )
        labels.append(
...
def inject_profile_deviation(wallets, start_time, n_events=10):
    ...
    for i in range(n_events):
        w = random.choice(wallets)
        # Upper-tail anomaly: 3–10× the wallet's own profile, not a separate distribution.
        # Multiplier reduced from [15,60] to [3,10] so amounts overlap the normal upper tail
        # (the anomaly signal is the spike relative to THIS wallet's history, not absolute size).
        ...
        rows.append(
            {
                ...
                "input_addresses": [w.address],
                "output_addresses": [receiver_addr],
                "input_amounts": [huge_amount],
                "output_amounts": [round(huge_amount - dev_fee, 8)],
                "fee": dev_fee,
                "script_type": w.script_type,
                "geo_country": w.asn_entry["country"],
                "asn": w.asn_entry["asn"],
        ...
        labels.append(
            {
                "txid": txid,
                "is_anomaly": 1,
                "anomaly_type": "profile_deviation",
...
def main():
    ...
    def rand_time():
        return start + timedelta(
            seconds=random.randint(0, int((end_time - start).total_seconds()))
        )

    for _ in range(6):
        r, l = inject_peeling_chain(
            wallets, rand_time(), chain_len=random.randint(5, 10)
        )
        all_rows += r
        all_labels += l
    for _ in range(4):
    ...
    tree.write(
        f"{args.out_dir}/transactions_sample.xml",
        encoding="utf-8",
        ...
        [
            {
                ...
                "home_ip": w.home_ip,
                "asn": w.asn_entry["asn"],
                "country": w.asn_entry["country"],
                "typical_amount_btc": w.typical_amount,
                "script_type": w.script_type,
            }
            for w in wallets
        ]
    ...
    print(
        f"Generated {len(df)} transactions ({n_anom} anomalous, {len(df) - n_anom} normal, "
        f"{n_anom / len(df) * 100:.2f}% anomaly rate)"
    )
    print(
        f"Anomaly type breakdown:\n{labels_df[labels_df.is_anomaly == 1].anomaly_type.value_counts()}"
    )
