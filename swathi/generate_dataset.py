...
from numba.parfors import parfor_lowering_utils
import argparse
import json
import random
...
def new_wallet_address():
    ...
    tail = f"{random.getrandbits(120):030x}"
...
def make_normal_transaction(wallets, t, wid_from=None):
    ...
    fee = round(amount * rand_fee_rate(), 8)
...
def inject_peeling_chain(wallets, start_time, chain_len=8):
    ...
    for hop in range(chain_len):
        ...
        fee = round(current_amount * rand_fee_rate(), 8)
...
def inject_fan_out_mixer(wallets, start_time, n_in=25, n_out=25):
    ...
    fanin_fee = round(total_in * rand_fee_rate(), 8)
    ...
    for i in range(n_out):
        ...
        txid_out = new_txid()
        out_fee = round(per_out * rand_fee_rate(), 8)
        rows.append({
...
def inject_structuring(wallets, start_time, threshold=10.0, n_tx=12):
    ...
    for i in range(n_tx):
        ...
        amount = round(threshold * (1 - np.random.uniform(0.005, 0.05)), 8)  # 0.5–5% below threshold
        receiver_addr = new_wallet_address()
        txid = new_txid()
        struct_fee = round(amount * rand_fee_rate(), 8)
        rows.append({
...
def inject_ip_hopping(wallets, start_time, n_tx=15):
    ...
    for i in range(n_tx):
        ...
        txid = new_txid()
        hop_fee = round(amount * rand_fee_rate(), 8)
        rows.append({
...
def inject_profile_deviation(wallets, start_time, n_events=10):
    ...
    for i in range(n_events):
        ...
        txid = new_txid()
        dev_fee = round(huge_amount * rand_fee_rate(), 8)
        rows.append({
            "timestamp": t.isoformat() + "Z", "src_ip": w.home_ip,
            "dst_ip": fake.ipv4_public(), "src_port": random.randint(1024, 65535),
            "dst_port": 8333, "txid": txid,
...
def main():
    ...
    # --- normal traffic, generated according to each wallet's own cadence ---
    for i in range(args.n_normal_tx):
        sender = random.choice(wallets)

        if sender.last_tx_time is None:
            tx_time = start + timedelta(
                minutes=random.uniform(0, 60 * 24 * 60)
            )
        else:
            tx_time = sender.last_tx_time + timedelta(
                minutes=np.random.exponential(sender.avg_gap_minutes)
            )

        row, _, _ = make_normal_transaction(
            wallets, tx_time, wid_from=sender.id
        )
